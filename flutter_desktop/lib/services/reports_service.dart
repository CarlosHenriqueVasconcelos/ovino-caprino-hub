...
class ReportsService {
  ...
  static Future<Map<String, dynamic>> getWeightsReport(
    ...
    final groupedWeights =
        _groupWeightsByAnimal(weights, animals, filters);

    final animalStats =
        _buildWeightStatistics(groupedWeights, animalMap);
    final summary =
        _buildWeightSummary(weights, groupedWeights, animalStats);
  ...
  static Future<Map<String, dynamic>> getVaccinationsReport(
    ...
    final database = await _resolveDatabase(db);
    final vaccinations =
        await _fetchVaccinationsWithAnimals(database, filters);
  ...
  static Future<Map<String, dynamic>> getMedicationsReport(
    ...
    final database = await _resolveDatabase(db);
    final medications =
        await _fetchMedicationsWithAnimals(database, filters);
  ...
  static Map<String, dynamic> _buildWeightSummary(
    ...
    return {
      'total_weighings': weights.isEmpty ? groupedWeights.length : weights.length,
  ...
  static Map<String, int> _buildNotesSummary(
  ...
  static Map<String, dynamic> _mapNoteRow(
    ...
    final animal =
        row['animal_id'] != null ? animals[row['animal_id']] : null;
    return {
      'date': row['date'],
      'title': row['title'],
      'category': row['category'],
